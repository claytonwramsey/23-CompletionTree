...
static Environment* buildAttemptEnv(const PickPlaceScenario* scenario, const std::vector<CPose>& poses,
                                     size_t movingIndex) {
...
HopPickPlaceNode::HopPickPlaceNode(const PickPlaceScenario* scenario, const std::vector<Action>* plan,
                                    RobotTag robot)
    : ComputeNode(nullptr), scenario(scenario), plan(plan), robot(robot), action_index(-1) {
  q_arm = hopcxx_pickplace_robot_q_start(scenario);
  poses = std::make_shared<std::vector<CPose>>();
  size_t n = hopcxx_pickplace_num_objects(scenario);
  poses->reserve(n);
  for (size_t i = 0; i < n; i++) poses->push_back(hopcxx_pickplace_object_pose(scenario, i));
  isComplete = true;
  isFeasible = true;
  isTerminal = false;
  name = "root";
...
void HopPickPlaceNode::untimedCompute() {
  ...
  if (!motionStarted) {
    ...
    if (act.type == ActionType::Pick) {
      ...
      if (ok) ok = rv.validate(qTarget, env);
      if (ok) {
      ...
      CHECK_EQ(held_object, (int64_t)act.object_index, "place must follow pick of the same object");
      float block_r = hopcxx_pickplace_block_r(scenario);
      int64_t goalSurface = hopcxx_pickplace_goal_surface(scenario);
      ...
      ok = rv.ik(eeTarget, &qTarget);
      if (ok) ok = rv.validate_attached(qTarget, env, block_r, heldRel);
      if (ok) {
        nextHeldObject = -1;
  ...
  size_t len = 0;
  float block_r = hopcxx_pickplace_block_r(scenario);
  int r = motionState.step(rv, q_arm, pendingQEnd, env, block_r, pendingHasHeld, pendingHeldRel, buf.data(),
                            maxWp, &len);
  hopcxx_env_free(env);

  if (r == 0) return;  // still searching -- leave isComplete false to be resumed later
  if (r < 0) {
    isFeasible = false;
    isComplete = true;
    return;
