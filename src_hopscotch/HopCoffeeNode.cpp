...
static size_t findItem(const CoffeeScenario* scenario, uint8_t kind, uint64_t index) {
  ...
  for (size_t i = 0; i < n; i++) {
    if (hopcxx_coffee_object_kind(scenario, i) == kind && hopcxx_coffee_object_index(scenario, i) == index) {
      return i;
    }
  }
...
static Environment* buildAttemptEnv(const CoffeeScenario* scenario, const std::vector<CPose>& poses,
                                     size_t movingIndex) {
  Environment* env = hopcxx_env_clone(hopcxx_coffee_env(scenario));
  float object_r = hopcxx_coffee_object_r(scenario);
  for (size_t i = 0; i < poses.size(); i++) {
    if (i == movingIndex) continue;
    hopcxx_env_add_ball(env, poses[i].x, poses[i].y, poses[i].z, object_r);
  }
  return env;
}
...
HopCoffeeNode::HopCoffeeNode(const CoffeeScenario* scenario, const std::vector<CoffeeAction>* plan,
                              RobotTag robot)
    : ComputeNode(nullptr), scenario(scenario), plan(plan), action_index(-1), robot(robot) {
  q_arm = hopcxx_coffee_robot_q_start(scenario);
  size_t n = hopcxx_coffee_num_objects(scenario);
  poses = std::make_shared<std::vector<CPose>>();
  poses->reserve(n);
  for (size_t i = 0; i < n; i++) poses->push_back(hopcxx_coffee_object_pose(scenario, i));
...
void HopCoffeeNode::untimedCompute() {
  ...
  if (!motionStarted) {
    ...
    Environment* env = buildAttemptEnv(scenario, *poses, act.item_index);
    CConfig qTarget;
    bool ok = true;
    ...
    if (act.type == CoffeeActionType::Pick) {
      ...
      switch (act.type) {
        ...
        case CoffeeActionType::Fill: {
          static thread_local std::mt19937 rng{std::random_device{}()};
          std::uniform_real_distribution<float> unit(0.0f, 2.0f * (float)M_PI);
          target = pose_mul(hopcxx_coffee_fill_pose(scenario), pose_from_xyz_yaw(0, 0, 0, unit(rng)));
          break;
        }
        case CoffeeActionType::Pour:
        case CoffeeActionType::Scoop:
        case CoffeeActionType::Stir:
          target = pose_mul(pose_mul(sampleZOff(object_r), (*poses)[act.target_index]), sampleZRot());
          break;
        case CoffeeActionType::Dump: {
          CPose flip = pose_from_xyz_rpy(0, 0, 0, (float)M_PI, 0, 0);
          target = pose_mul(pose_mul(pose_mul(sampleZOff(object_r), (*poses)[act.target_index]), sampleZRot()),
                             flip);
          break;
        }
    ...
    if (ok) {
      ...
      ok = hasHeld ? rv.validate_attached(qTarget, env, object_r, heldRel) : rv.validate(qTarget, env);
    }
    if (ok) diagStats().validateOk[action_index]++;
    hopcxx_env_free(env);
    if (!ok) {
      isFeasible = false;
      isComplete = true;
  ...
  size_t len = 0;
  int r = motionState.step(rv, q_arm, pendingQEnd, env, object_r, pendingHasHeld, pendingHeldRel, buf.data(),
                            maxWp, &len);
  hopcxx_env_free(env);
  ...
  for (size_t w = 0; w < len; w++) {
    CConfig c{};
    c.dim = nextQArm.dim;
    for (int d = 0; d < c.dim; d++) c.q[d] = buf[w * c.dim + d];
  ...
  switch (act.type) {
    ...
    case CoffeeActionType::Fill: {
      ...
      CHECK(!(*nl)[act.item_index].coffee && !(*nl)[act.item_index].mixed, "fill precondition violated");
    ...
    case CoffeeActionType::Scoop: {
      ...
      CHECK((*cupLogic)[act.target_index].sugar, "scoop precondition violated (cup has no sugar)");
      spoonScooped = true;
      break;
    }
    case CoffeeActionType::Dump: {
  ...
  isComplete = true;
  l = 1.;
  if (action_index + 1 == (int)plan->size()) isTerminal = true;
}
