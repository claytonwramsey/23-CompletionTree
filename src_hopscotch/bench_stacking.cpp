...
static void dumpSolution(const char* path, const rai::String& robotName, uint64_t seed,
  ...
  for (HopStackingNode* n = solutionNode; n->action_index >= 0; n = dynamic_cast<HopStackingNode*>(n->parent)) {
    chain.push_back(n);
  }
  ...
  os <<"  \"q_start\": "; writeConfig(os, hopcxx_stacking_robot_q_start(scenario)); os <<",\n";
  os <<"  \"actions\": [\n";
...
int main(int argc, char** argv) {
  ...
  if (robot == RobotTag::Panda) scenario = hopcxx_make_stacking_scenario_panda(seed);
  else if (robot == RobotTag::Ur5) scenario = hopcxx_make_stacking_scenario_ur5(seed);
  else scenario = hopcxx_make_stacking_scenario_pr2(seed);
  ...
  printf("problem=stacking robot=%s seed=%llu solved=%d elapsed_s=%.4f steps=%u nodes=%u plan_len=%zu\n",
