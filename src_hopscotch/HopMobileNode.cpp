...
static Environment* buildAttemptEnv(const MobileScenario* scenario, const std::vector<CPose>& poses,
                                     size_t movingIndex, CPose bq) {
...
HopMobileNode::HopMobileNode(const MobileScenario* scenario, const std::vector<Action>* plan)
    : ComputeNode(nullptr), scenario(scenario), plan(plan), action_index(-1) {
  ...
  size_t n = hopcxx_mobile_num_objects(scenario);
  poses->reserve(n);
  for (size_t i = 0; i < n; i++) poses->push_back(hopcxx_mobile_object_pose(scenario, i));
  isComplete = true;
  isFeasible = true;
  isTerminal = false;
  name = "root";
