...
struct CoffeeAction {
  ...
  size_t target_index;  // reference item for Pour/Scoop/Dump/Stir's "over ___" pose (unused otherwise)
};
...
struct HopCoffeeNode : rai::ComputeNode {
  ...
  std::shared_ptr<std::vector<CPose>> poses;      // per scenario object index (only Pick/Place move these)
  std::shared_ptr<std::vector<CupLogic>> cupLogic;  // per scenario object index (only meaningful for cups)
  ...
  HopCoffeeNode(const CoffeeScenario* scenario, const std::vector<CoffeeAction>* plan, RobotTag robot);
