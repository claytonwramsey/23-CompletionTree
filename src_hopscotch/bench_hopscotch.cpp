...
static void dumpSolution(const char* path, const rai::String& problem, const rai::String& robotName,
  ...
  for (HopPickPlaceNode* n = solutionNode; n->action_index >= 0; n = dynamic_cast<HopPickPlaceNode*>(n->parent)) {
    chain.push_back(n);
  }
  ...
  for (size_t i = 0; i < ns; i++) {
    ...
    os <<"{\"height\": " <<t.height <<", \"aabb\": [" <<t.x0 <<"," <<t.y0 <<"," <<t.x1 <<"," <<t.y1 <<"]}";
  }
  os <<"],\n";
  os <<"  \"block_r\": " <<hopcxx_pickplace_block_r(scenario) <<",\n";
  os <<"  \"q_start\": "; writeConfig(os, hopcxx_pickplace_robot_q_start(scenario)); os <<",\n";
  os <<"  \"actions\": [\n";
...
int main(int argc, char** argv) {
  ...
  if (problem == "cabinet") {
    if (robot == RobotTag::Panda) scenario = hopcxx_make_cabinet_scenario_panda(seed);
    else if (robot == RobotTag::Ur5) scenario = hopcxx_make_cabinet_scenario_ur5(seed);
    else scenario = hopcxx_make_cabinet_scenario_pr2(seed);
  } else if (problem == "packing") {
    if (robot == RobotTag::Panda) scenario = hopcxx_make_packing_scenario_panda(seed);
    else if (robot == RobotTag::Ur5) scenario = hopcxx_make_packing_scenario_ur5(seed);
    else scenario = hopcxx_make_packing_scenario_pr2(seed);
  } else {
  ...
  printf("problem=%s robot=%s seed=%llu solved=%d elapsed_s=%.4f steps=%u nodes=%u plan_len=%zu\n",
