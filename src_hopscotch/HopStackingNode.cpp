...
std::vector<Action> makeStackingPlan(const StackingScenario* scenario) {
  ...
  for (size_t k = 0; k < n; k++) {
    ...
    for (size_t i = 0; i < n; i++) {
      if (hopcxx_stacking_object_id(scenario, i) == goalId) { idx = i; break; }
    }
...
static Environment* buildAttemptEnv(const StackingScenario* scenario, const std::vector<CPose>& poses,
                                     size_t movingIndex) {
...
HopStackingNode::HopStackingNode(const StackingScenario* scenario, const std::vector<Action>* plan,
  ...
  size_t n = hopcxx_stacking_num_objects(scenario);
  poses->reserve(n);
  for (size_t i = 0; i < n; i++) poses->push_back(hopcxx_stacking_object_pose(scenario, i));
  isComplete = true;
  isFeasible = true;
  isTerminal = false;
  name = "root";
    ...
    if (act.type == ActionType::Pick) {
      ...
      if (towerPos == 0) {
        ...
        for (size_t i = 0; i < hopcxx_stacking_num_objects(scenario); i++) {
          if (hopcxx_stacking_object_id(scenario, i) == belowId) { belowIdx = i; break; }
        }
