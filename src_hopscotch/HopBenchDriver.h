...
inline TrialResult runSearch(rai::AStar& astar, double timeout_s, int stepCap, int nodeCap) {
  ...
  while (true) {
    ...
    if (astar.solutions.N > 0) { r.solved = true; break; }
    if (r.elapsed_s >= timeout_s) break;
    if ((int)astar.steps >= stepCap) break;
    if ((int)astar.mem.N >= nodeCap) break;
    if (!astar.queue.N) break;
  }
  r.steps = astar.steps;
  r.nodes = astar.mem.N;
  return r;
...
inline void writePose(std::ostream& os, CPose p) {
  os <<"[" <<p.x <<"," <<p.y <<"," <<p.z <<"," <<p.qx <<"," <<p.qy <<"," <<p.qz <<"," <<p.qw <<"]";
}
inline void writeConfig(std::ostream& os, CConfig c) {
  os <<"[";
  for (int i = 0; i < c.dim; i++) { if (i) os <<","; os <<c.q[i]; }
  os <<"]";
}
