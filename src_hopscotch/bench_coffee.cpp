...
static RobotTag parseRobot(const char* s) {
  if (!strcmp(s, "panda")) return RobotTag::Panda;
  if (!strcmp(s, "ur5")) return RobotTag::Ur5;
  if (!strcmp(s, "pr2")) return RobotTag::Pr2;
  HALT("unknown robot '" <<s <<"'");
}

static const char* actionName(CoffeeActionType t) {
  switch (t) {
    case CoffeeActionType::Pick: return "pick";
    case CoffeeActionType::Place: return "place";
    case CoffeeActionType::Fill: return "fill";
    case CoffeeActionType::Pour: return "pour";
    case CoffeeActionType::Scoop: return "scoop";
    case CoffeeActionType::Dump: return "dump";
    case CoffeeActionType::Stir: return "stir";
  }
  return "?";
}
...
static void dumpSolution(const char* path, const rai::String& robotName, uint64_t seed,
  ...
  for (HopCoffeeNode* n = solutionNode; n->action_index >= 0; n = dynamic_cast<HopCoffeeNode*>(n->parent)) {
    chain.push_back(n);
  }
  ...
  for (size_t i = 0; i < n; i++) {
    if (i) os <<", ";
    os <<"{\"kind\": " <<(int)hopcxx_coffee_object_kind(scenario, i) <<", \"index\": "
       <<hopcxx_coffee_object_index(scenario, i) <<", \"start_pose\": ";
  ...
  os <<"  \"q_start\": "; writeConfig(os, hopcxx_coffee_robot_q_start(scenario)); os <<",\n";
  os <<"  \"actions\": [\n";
  for (size_t i = 0; i < chain.size(); i++) {
    HopCoffeeNode* node = chain[i];
    const CoffeeAction& act = plan[node->action_index];
    os <<"    {\"type\": \"" <<actionName(act.type) <<"\", \"item_index\": " <<act.item_index
       <<", \"trajectory\": [";
...
int main(int argc, char** argv) {
  ...
  if (robot == RobotTag::Panda) scenario = hopcxx_make_coffee_scenario_panda(seed);
  else if (robot == RobotTag::Ur5) scenario = hopcxx_make_coffee_scenario_ur5(seed);
  else scenario = hopcxx_make_coffee_scenario_pr2(seed);
  ...
  printf("problem=coffee robot=%s seed=%llu solved=%d elapsed_s=%.4f steps=%u nodes=%u plan_len=%zu\n",
         robotName.p, (unsigned long long)seed, result.solved ? 1 : 0, result.elapsed_s, result.steps,
  ...
  if (rai::getParameter<bool>("diagStats", false)) {
    ...
    for (size_t i = 0; i < plan.size(); i++) {
      printf("  [%zu] %-6s item=%zu: attempts=%lld ikOk=%lld validateOk=%lld motionOk=%lld\n", i,
             actionName(plan[i].type), plan[i].item_index, diagStats().attempts[i], diagStats().ikOk[i],
             diagStats().validateOk[i], diagStats().motionOk[i]);
    }
  }
