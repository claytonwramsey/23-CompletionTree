...
#define HOPCXX_VTABLE_TRAMPOLINE(prefix)                                                     \
  inline void* prefix##_motion_plan_start(CConfig q_start, CConfig q_end, const Environment* env, \
                                           float block_r, bool has_held, CPose held_rel_pose) {    \
    return hopcxx_##prefix##_motion_plan_start(q_start, q_end, env, block_r, has_held, held_rel_pose); \
  }                                                                                            \
  inline int prefix##_motion_plan_resume(void* handle, size_t n_samples, float* out_buf,       \
                                          size_t out_cap, size_t* out_len) {                    \
    return hopcxx_##prefix##_motion_plan_resume(                                               \
        reinterpret_cast<hopcxx_##prefix##_Planning*>(handle), n_samples, out_buf, out_cap, out_len); \
  }                                                                                             \
  inline void prefix##_motion_plan_free(void* handle) {                                        \
    hopcxx_##prefix##_motion_plan_free(reinterpret_cast<hopcxx_##prefix##_Planning*>(handle));  \
  }
...
inline const RobotVtable& robot_vtable(RobotTag tag) {
  static const RobotVtable panda{
      hopcxx_panda_ik,       hopcxx_panda_fk,
      hopcxx_panda_validate, hopcxx_panda_validate_attached,
      hopcxx_panda_sample_rel_pose, hopcxx_panda_sample_table_pose,
      hop_vtable_detail::panda_motion_plan_start, hop_vtable_detail::panda_motion_plan_resume,
      hop_vtable_detail::panda_motion_plan_free,
  };
  static const RobotVtable ur5{
      hopcxx_ur5_ik,       hopcxx_ur5_fk,
      hopcxx_ur5_validate, hopcxx_ur5_validate_attached,
      hopcxx_ur5_sample_rel_pose, hopcxx_ur5_sample_table_pose,
      hop_vtable_detail::ur5_motion_plan_start, hop_vtable_detail::ur5_motion_plan_resume,
      hop_vtable_detail::ur5_motion_plan_free,
  };
  static const RobotVtable pr2{
      hopcxx_pr2_ik,       hopcxx_pr2_fk,
      hopcxx_pr2_validate, hopcxx_pr2_validate_attached,
      hopcxx_pr2_sample_rel_pose, hopcxx_pr2_sample_table_pose,
      hop_vtable_detail::pr2_motion_plan_start, hop_vtable_detail::pr2_motion_plan_resume,
      hop_vtable_detail::pr2_motion_plan_free,
  };
  switch (tag) {
    case RobotTag::Panda: return panda;
    case RobotTag::Ur5: return ur5;
    case RobotTag::Pr2: return pr2;
  }
