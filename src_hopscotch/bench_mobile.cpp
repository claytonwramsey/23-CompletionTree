...
static void dumpSolution(const char* path, uint64_t seed, const MobileScenario* scenario,
  ...
  for (HopMobileNode* n = solutionNode; n->action_index >= 0; n = dynamic_cast<HopMobileNode*>(n->parent)) {
    chain.push_back(n);
  }
  ...
  os <<"  \"q_start\": "; writeConfig(os, hopcxx_mobile_robot_q_start_arm(scenario)); os <<",\n";
  os <<"  \"base_start_pose\": "; writePose(os, hopcxx_mobile_base_start_pose(scenario)); os <<",\n";
  os <<"  \"actions\": [\n";
...
int main(int argc, char** argv) {
  ...
  printf("problem=mobile robot=pr2 seed=%llu solved=%d elapsed_s=%.4f steps=%u nodes=%u plan_len=%zu\n",
         (unsigned long long)seed, result.solved ? 1 : 0, result.elapsed_s, result.steps, result.nodes,
         plan.size());

  rai::String dumpPath = rai::getParameter<rai::String>("dumpSolution", STRING(""));
  if (result.solved && dumpPath.N) {
    dumpSolution(dumpPath.p, seed, scenario, plan, dynamic_cast<HopMobileNode*>(astar.solutions(0)));
  }
